...
#     # This function converts a tensor image array to a numpy image array.
#     # tensor[0].cpu().float().numpy() converts the image into a numpy array
#     image = (255.0/2.0) * (tensor[0].cpu().float().numpy() + 1.0)
#     # this operation is more like --> (255)*((numpy-image + 1)/2.0). This is need because the tensor values are between
#     # [-1,1] and not [0,1]. Therefore, to properly convert the tensor to numpy array we must also make sure their values
#     # are also between [0,1].
#     if image.shape[0] == 1: # conversion from grayscale to RGB
