...
# from cycleGAN3D.losses import gradient_consistency_loss
...
from losses import gradient_consistency_loss
...
save_path = '/home/naga/PycharmProjects/deepLearning/cycleGAN3D/saved_models/mr2ct_unet_gradient_consistency'
...
for epoch in range(args.epoch, args.n_epochs+1):
    ...
    for i, batch in enumerate(dataloader):
        ...
        loss_cycle_A2B2A = 3.0 * criterion_cycle(recovered_A, real_A)   # lambda_cycle = 3.0
        recovered_B = netG_A2B(fake_A)
        loss_cycle_B2A2B = 3.0 * criterion_cycle(recovered_B, real_B)

        loss_cycle_consistency = loss_cycle_A2B2A + loss_cycle_B2A2B
        ...
        loss_gradient_consistency = 0.3*(loss_gc_A + loss_gc_B)   # 0.3 is the lambda_gc value in paper
