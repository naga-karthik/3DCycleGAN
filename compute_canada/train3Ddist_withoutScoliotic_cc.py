...
from models3D_lighter_cc import LighterResUnetGenerator3D, LighterUnetGenerator3D, PatchGANDiscriminatorwithSpectralNorm
...
save_path = '/home/karthik7/projects/def-laporte1/karthik7/cycleGAN3D/saved_models/' \
            'mr2ct_v1_withoutScoliotic_default'
    ...
    for epoch in range(args.epoch, args.n_epochs+1):
        ...
        for i, batch in enumerate(dataloader):
            ...
            if (i+1) % 70 == 0:
                ...
                tempA = fake_A[0].cpu().float().numpy()
                tempA.squeeze(0)
            ...
            if rank == 0:
                ...
                # if (i+1) % 3 == 0:
                #     torch.save(netG_A2B.state_dict(), save_path+"/netG_A2B_%d.pth" % (i+1))
                #     torch.save(netG_B2A.state_dict(), save_path+"/netG_B2A_%d.pth" % (i+1))
                #     torch.save(netD_A.state_dict(), save_path+"/netD_A_%d.pth" % (i+1))
                #     torch.save(netD_B.state_dict(), save_path+"/netD_B_%d.pth" % (i+1))
...
def run_train_model(train_func, world_size):
    ...
    parser.add_argument('--dataroot', type=str, default=path + 'mr2ct_newData_withoutScoliotic/',
