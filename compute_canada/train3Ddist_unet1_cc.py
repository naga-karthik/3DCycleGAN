"""
The following code assumes that only 1 node is being used and that node contains multiple GPUs.
"""
import argparse
import itertools
import os, sys
...
def train_model(rank, args):
    ...
    # THE NETWORKS
    netG_A2B = UnetGenerator3DUpdated(args.input_nc, args.output_nc).to(rank)
    netG_B2A = UnetGenerator3DUpdated(args.output_nc, args.input_nc).to(rank)
    ...
    # this is torch.distributed's own data loader method which loads the data such that they are non-overlapping
    # and decides which process uses which slice of the data.
    data_sampler = torch.utils.data.distributed.DistributedSampler(dataset=data, num_replicas=args.world_size,
                                                                   rank=rank)
    dataloader = DataLoader(dataset=data, batch_size=args.batch_size, shuffle=False, num_workers=args.n_cpu,
...
def run_train_model(train_func, world_size):
    ...
    # the spawn method calls the train_func "nprocs" number of times, for each GPU.
    # the "world_size" is nothing but the total number of processes to be run.
    # the best practice is to use a single process for a single GPU. One process can handle multiple GPUs also
    # but it is not fully efficient.
