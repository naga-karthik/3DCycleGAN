# Attempt to use multiple GPUs to run code parallely
...
parser.add_argument('--gpu_ids', type=str, default='0', help='gpu ids to be used e.g. 0, 0,1,2 or 0,2.')
...
def init_net_gpu(net, gpu_ids=[]):
    # function for making the model ready for multiple-GPU processing
    if len(gpu_ids) > 0:
...
# using the GPUs
netG_A2B = init_net_gpu(netG_A2B, args.gpu_ids)
netG_B2A = init_net_gpu(netG_B2A, args.gpu_ids)
netD_A = init_net_gpu(netD_A, args.gpu_ids)
netD_B = init_net_gpu(netD_B, args.gpu_ids)
...
input_A = Tensor(args.batch_size, args.input_nc, args.size_z, args.size_x,
                 args.size_y)  # defining the size of the input here
input_B = Tensor(args.batch_size, args.output_nc, args.size_z, args.size_x,
                 args.size_y)  # defining the size of the output here
