...
class UnetDownBlock(nn.Module):
    ...
    def __init__(self, in_channels, features, name):
        ...
        self.unet_downblock = nn.Sequential(
            OrderedDict(
                [(name + 'conv1',
                  spectral_norm(nn.Conv3d(in_channels=in_channels, out_channels=features, kernel_size=3, stride=2,
...
#   1) Kernel size is changed from 4 to 3 in UnetDownBlock for solving "kernel size-is-larger-than-the-input" issue.
...
class UnetGenerator3DwithSpectralNorm (nn.Module):

    def __init__(self, in_channels, out_channels, init_featMaps=32):
        super(UnetGenerator3DwithSpectralNorm, self).__init__()
        ...
        self.upconv5 = spectral_norm(nn.ConvTranspose3d(in_channels=features * 16, out_channels=features * 16, kernel_size=3, stride=2, padding=1, output_padding=(1,1,1)))
        self.decoder5 = UnetUpBlock(features * 16, name='dec5')
        self.upconv4 = spectral_norm(nn.ConvTranspose3d(in_channels=features * 16, out_channels=features * 8, kernel_size=3, stride=2, padding=1, output_padding=(0,1,1)))
        self.decoder4 = UnetUpBlock(features * 8, name='dec4')  # it is here that outputs from contractive path will be concatenated.
        self.upconv3 = spectral_norm(nn.ConvTranspose3d(in_channels=features * 8, out_channels=features * 4, kernel_size=3, stride=2, padding=1, output_padding=(1,1,1)))
        self.decoder3 = UnetUpBlock(features * 4, name='dec3')
        self.upconv2 = spectral_norm(nn.ConvTranspose3d(in_channels=features * 4, out_channels=features * 2, kernel_size=3, stride=2, padding=1, output_padding=(1,1,1)))
        self.decoder2 = UnetUpBlock(features * 2, name='dec2')
        self.upconv1 = spectral_norm(nn.ConvTranspose3d(in_channels=features * 2, out_channels=features, kernel_size=3, stride=2, padding=1, output_padding=(1,1,1)))
        self.decoder1 = UnetUpBlock(features, name='dec1')

        self.conv = spectral_norm(nn.ConvTranspose3d(in_channels=features, out_channels=out_channels, kernel_size=3, stride=2, padding=1, output_padding=(1,1,1)))
