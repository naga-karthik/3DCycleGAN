...
from losses_cc import normalized_mutual_info_loss
...
save_path = '/home/karthik7/projects/def-laporte1/karthik7/cycleGAN3D/saved_models/' \
            'mr2ct_lighterUnet_nmi_batchSize4'
...
def train_model(rank, args):
    ...
    for epoch in range(args.epoch, args.n_epochs+1):
        ...
        for i, batch in enumerate(dataloader):
            ...
            loss_nmi_A = normalized_mutual_info_loss(real_img=real_A, fake_img=fake_B)
            loss_nmi_B = normalized_mutual_info_loss(real_img=real_B, fake_img=fake_A)

            loss_normalized_mi = 1.0 * (loss_nmi_A + loss_nmi_B)

            # COMBINING ALL LOSSES FOR THE GENERATOR NOW
            loss_G = adversarial_loss + loss_cycle_consistency + loss_normalized_mi
            loss_G.backward()
            ...
            if rank == 0:
                logger.log(save_path, {'G Adversarial Loss': adversarial_loss,
                                       ...
                                       'G Normalized MI Loss': loss_normalized_mi,
                                       'G Loss': loss_G,
