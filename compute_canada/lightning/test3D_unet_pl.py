...
class LightningCycleGAN3D(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        lr_scheduler_G = optim.lr_scheduler.LambdaLR(optimizer_G, lr_lambda=LambdaLR(args.n_epochs, args.epoch,
                                                                                     args.decay_epoch).step)
        lr_scheduler_D_A = optim.lr_scheduler.LambdaLR(optimizer_D_A, lr_lambda=LambdaLR(args.n_epochs, args.epoch,
                                                                                         args.decay_epoch).step)
        lr_scheduler_D_B = optim.lr_scheduler.LambdaLR(optimizer_D_B, lr_lambda=LambdaLR(args.n_epochs, args.epoch,
                                                                                         args.decay_epoch).step)
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            loss_cycle_A2B2A = self.cycle_loss(recovered_A, real_A)  # lambda_cycle = 3.0
            recovered_B = self.netG_A2B(fake_A)
            loss_cycle_B2A2B = self.cycle_loss(recovered_B, real_B)

            loss_cycle_consistency = 3.0 * (loss_cycle_A2B2A + loss_cycle_B2A2B)
            ...
            loss_gradient_consistency = 1.0 * (loss_gc_A + loss_gc_B)
...
if __name__ == '__main__':
    ...
    # only for testing
    v_num, epoch_num = 6, 199
    checkpoint_path = log_path+'cyclegan3d_16bit/version_'+str(v_num)+'/checkpoints/epoch='+str(epoch_num)+'.ckpt'
