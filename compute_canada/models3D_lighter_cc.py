...
class NormalConv3DBlock(nn.Module):
    def __init__(self, in_features, out_features, residual=None):
...
class LighterUnetGenerator3D(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=1, num_feat_maps=[32, 64, 128, 256]):
        ...
        # Downsampling Path
        self.conv_block1 = Conv3DBlock(in_channels, num_feat_maps[0])  # does not downsample in the 1st block
        self.conv_block2 = Conv3DBlock(num_feat_maps[0], num_feat_maps[1])
        self.conv_block3 = Conv3DBlock(num_feat_maps[1], num_feat_maps[2])
        self.conv_block4 = Conv3DBlock(num_feat_maps[2], num_feat_maps[3])
