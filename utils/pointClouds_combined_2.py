            ...
            for j in range(3, 76):
                if not lines[i+j].startswith("#"):  # ignore lines that start with '#'
                    ...
                    if len(data) == 5:
                        ...
                        # x_coord.append(float(data[1]) + 0.55124138)
                        # y_coord.append(float(data[3]) + 0.15411283)
...
# print(array, "\n", array.min(), array.max(), array.shape)
# sys.exit()
...
# print("Landmark Mean Coordinates Before translation: \n", mean_vec)
...
# Scaling the values between 0 and 1.
m1, n1 = vol_data.shape
vol_data = vol_data.reshape(-1,1)
vol_data = 2 * (vol_data - vol_data.min())/(vol_data.max() - vol_data.min()) - 1
vol_data = vol_data.reshape(m1, n1)
# print(vol_data, "\n", vol_data.min(), vol_data.max(), vol_data.shape)
# sys.exit()
...
# ------- TRANSLATION ----------
trans_x = seg_mean_x - mean_x
trans_y = seg_mean_y - mean_y
trans_z = seg_mean_z - mean_z
trans_vec = np.array([[trans_x], [trans_y], [trans_z]])
array = array + trans_vec   # translation
# ---------------------
...
# print("Landmark Mean Coordinates after translation: \n", land_mean_vec)
...
# the difference between scatter matrix and covariance matrix is the scaling factor. Cov. Mat is the scaled version
scatter_matrix_landmarks = (1.0/array.shape[1])*scatter_matrix_landmarks

# eigenvalues and eigenvectors of the scatter matrix
land_eig_val_sc, land_eig_vec_sc = np.linalg.eig(scatter_matrix_landmarks)
print("Eigenvalues from scatter (now covariance) matrix: \n", land_eig_val_sc)
...
# the difference between scatter matrix and covariance matrix is the scaling factor. Cov. Mat is the scaled version
scatter_matrix_segment = (1.0/vol_data.shape[1])*scatter_matrix_segment


# eigenvalues and eigenvectors of the scatter matrix
seg_eig_val_sc, seg_eig_vec_sc = np.linalg.eig(scatter_matrix_segment)
print("Eigenvalues from scatter (now covariance) matrix segmented: \n", seg_eig_val_sc)
